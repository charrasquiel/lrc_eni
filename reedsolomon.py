...
def divide_en_shards(array_codificado, num_shards):
    # Calcula el tamaño aproximado de cada shard
    shard_size = len(array_codificado) // num_shards
    inicio = 0
...
def recuperar_array(desde_directorio):
    shards = []
    for filename in os.listdir(desde_directorio):
        if filename.endswith(".shard"):
            shard_path = os.path.join(desde_directorio, filename)
